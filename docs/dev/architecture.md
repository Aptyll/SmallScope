...
| [js/sprites/core.js](../../js/sprites/core.js) | ~175 | `SPRITES`, `SPR` | the empty sprite registry and the bake helpers (`bake`/`bakeSpan`/`flipH`/`bakeClips`/`liveIcon`/`wash`/`double`) plus the `TEAM_SKINS` table; every other sprite file is a private IIFE that bakes its grids and `Object.assign`s the keys it owns into `SPRITES` |
...
| [js/sprites/terrain.js](../../js/sprites/terrain.js) | ~1700 | → `SPRITES` | the pine and its 24 wind frames + the one atlas, stumps, the dead gold ore and mine, the bush, the dead snags, the den, the hog hut |
| [js/sprites/rocks.js](../../js/sprites/rocks.js) | ~200 | → `SPRITES` | the three rock kinds, their rubble, the mining channel's crack overlays and the glint sites |
...
| [js/depth.js](../../js/depth.js) | ~260 | shared scope, no `window.*` export | the snow's depth: the seed's prevailing wind, the drifts laid in the lee of the scenery (`layDrifts`, at boot after `placeRocks`), `snowDepth`/`deepAt`, and wading (`wadeStep`, `wadeMul`) - the detail: [world](world.md#snow-depth) |
...
| [js/mining.js](../../js/mining.js) | ~290 | shared scope, no `window.*` export | the rocks as a thing to mine: the `ROCK_KINDS` table, the ores as carried items, the channel on the work key, and the forge's rules (a weapon's forge points and level, what a level adds, what ore is worth, the order that dumps a pile into it) |
...
| [js/draw/lakes.js](../../js/draw/lakes.js) | ~560 | shared scope, no `window.*` export | the wind's leavings, laid at the ground bake: drifts in the lee of what stands, the lakes' dust, downwind banks, long cracks and reeds, and the weather's per-frame pass over the ice (`drawLakeSky`) - the detail: [world](world.md#the-winds-leavings) |
| [js/draw/trample.js](../../js/draw/trample.js) | ~290 | shared scope, no `window.*` export | the trampled snow: the packed/churned grid every body stamps, its refill, and the chunk canvases that draw it under the footprints |
...
| [js/draw/depth.js](../../js/draw/depth.js) | ~100 | shared scope, no `window.*` export | deep snow's pixels: `DEEP_PAL` (loaded before ground.js, whose `SNOW_INK` appends it), `deepTone` (per pixel, from `snowTile`) and a body wading (`drawWading`, from `render()`'s body dispatch) |
...
| [js/ui/shop.js](../../js/ui/shop.js) | ~1900 | shared scope, no `window.*` export | the merchant's counter: the fish/berry market and its three-day history, the rolled stock and its turnover, buying and selling, and the panel all three are read on |
| [js/ui/forge.js](../../js/ui/forge.js) | ~390 | shared scope, no `window.*` export | the counter's FORGE face: the SHOP / FORGE tab plates, the bench (weapon well, pile well, forge bar, preview, the plate that forges, the ore plates), its hits, drops and tooltips, and the `+N` mark a forged weapon wears in every well |
...
Eleven files: `core.js` loads first and makes the empty `window.SPRITES` registry and `window.SPR`
(the bake helpers); each of the ten art files is a **private IIFE** that ends with
...
trimmed and box-filtered down `SHRINK` (four) times - colour and coverage averaged per cell, so
an edge lands soft (a hard one-pixel sample broke the painting's shading into an uneven mess) -
to the size the title draws it at 1:1, and written as
