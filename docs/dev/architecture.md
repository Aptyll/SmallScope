...
trimmed and sampled down `SHRINK` (three) times, one source pixel per baked pixel with a hard
edge (averaging blurred it), snapped to a `PALETTE_N`-colour palette clustered from its own
pixels (the painting shades its edges; a logo wants flat bands), to the size the title draws
it at 1:1, and written as
