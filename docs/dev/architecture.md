...
| [js/mining.js](../../js/mining.js) | ~260 | shared scope, no `window.*` export | the rocks as a thing to mine: the `ROCK_KINDS` table, the ores as carried items, the channel on the work key, and the forge's rules (a weapon's level, what it adds, what each level costs, the order that spends the ore) |
...
| [js/ui/forge.js](../../js/ui/forge.js) | ~390 | shared scope, no `window.*` export | the counter's FORGE face: the SHOP / FORGE tab plates, the bench (weapon well, ore well, preview, the plate that forges, the ladder of costs), its hits, drops and tooltips, and the `+N` mark a forged weapon wears in every well |
