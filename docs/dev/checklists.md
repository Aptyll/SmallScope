...
| js/mining.js | `ROCK_KINDS` (each rock kind's channel, gold, ore, find and regrow), `ORE_PRICE`, `MINE_STRIKE`/`MINE_MOVE`/`ORE_FLING`, the forge's `FORGE_PTS`/`FORGE_NEED`/`FORGE_NEED_UP`/`FORGE_TAPER`/`FORGE_DMG`/`FORGE_ROF`/`FORGE_TENSILE`/`FORGE_GOLD` (and each body's `up`, `TOOLS` in js/tools.js) |
