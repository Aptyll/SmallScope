...
| the pre-match build: the hero pop-up off your figure on the lobby - live preview, stat ledger with hover deltas, the twelve 32×32 gear wells, the equip flash, the stat points' pips and +/- plates, the four ability wells that raise their card | `heroLayout`, `heroScreenHit`, `pickGear`, `spendPt`, `renderHero`, `drawGearWell`, `drawGearPreview`, `heroPreviewKit`, `GEAR32`/`gearIcon32`, `beginHero`/`leaveHero`, `HERO_*` | `main menu` › `the hero pop-up` (the numbers' base: `baseKit`; the tracks and the budget: `STAT_TRACKS`/`STAT_POINTS`/`ptsSpent`, player.js `stat points`) |
