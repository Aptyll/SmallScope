...
| the rooms screen: the relay's open rooms as planks under HOST, a join that reloads onto the room's seed, the waiting room it opens | `RM_*`, `roomsFeed`, `beginRooms`, `leaveRooms`, `roomsLayout`, `roomOk`, `roomsHit`, `hostRoom`, `joinRoom`, `roomsKey`, `roomsClick`, `updateRooms`, `renderRooms` | `rooms` (js/ui/menu.js) |
