...
| the seed die on class select (under the map), and the reroll | `drawSeedDie`, `rerollWorld`, `selectLayout().seed` | `main menu` › `class select` |
| class select: the painted night, the two team panels glued to the screen's edges (your side left, rivals right, face-down until the count), the character tabs heading yours, the rivals' three difficulty plates and the target whose arrows fly in per level, the map at the top centre with its chevrons and the seed row, your character on stage with the gear widget, LOCK IN at the foot and the five-second count it wears (a second press skips it) | `SEL_*`, `selectLayout`, `selectHit`, `CLASS32`/`classIcon32` (and the rail's 12px `CLASS12`/`classIcon12` beside them), `drawSelectBackdrop`, `drawSelectSlot`, `drawSelectStage`, `drawSelectCard`, `drawSelectRosters`, `pxDisc`, `drawSelectTarget`, `drawSelectDiff`, `drawChevron`, `drawSelectMap`, `drawSelectCount`, `renderSelect`, `selectSlot`/`selectStep`, `mapStep`, `pressPlay`, `cancelCount`, `selectRevealed`, `setAiLevel`, `lockIn`, `COUNT_T`, `LOCK_FX_T`, `TGT_R`/`TGT_ANG` | `main menu` › `class select` (the levels' names: `AI_LEVELS`, ai.js; the slots' store: `PROFILE`, profile.js; the shape and the page: `mapChip`/`pickMap`, `the map`) |
