...
The game code is ~34000 lines of flat top-level code across the flat files of js/ (with js/net/, js/draw/ and js/ui/ under it) sharing one global
...
| the rebindable actions, their defaults per scheme, the two bind maps and the live one, how anything asks for a key, and a pad's held actions (`actHeld`, written by gamepad.js) | `KEY_ACTIONS`/`KEY_ACT` (`key` / `ck`), `SCHEMES`, `schemeKey`/`schemeActs`, `settings.binds`/`settings.bindsClick`/`binds()`, `mendBinds`, `actKey`, `actHeld`, `keyIs`/`keyHeld`/`keyBound`, `moveDir`, `keyCap`/`keyCapShort`, `keyReserved` | `keys and binds` |
...
| the bare gestures a pad's trigger sends: the draw, the flag wheel, a build wheel with no tile under a pointer, a page scroll | `fireDown`/`fireUp`, `openFlagWheel`, `openWheelNear`, `panelScrollBy` | `input` |
...
| is this tile open water (an ice hole or the creek) - the one read every walker, route, spawn and climb-out asks | `waterAt` | `world` (under `isSolidTile`) |
...
| the zipline: its tuning, where each side's cable runs, laying it at boot, a point along a line and its height, the nearest point to a body, whether a body may clip on, and the ride's verbs (clip on / let go, the per-step ride, the knocked-off drop) | `ZIP_*`, `zips`, `zipTile`, `placeZips` (called at boot after `placeRoad`, boot.js), `zipPoint`, `zipLift`, `zipNearest`, `zipNear`, `zipUnder` (the cable as drawn under a world point: the hover, `hoverZip`, actions.js, and the click scheme's press), `zipToggle` (what `input.jump` does on the ground: `updatePlay`, sim.js), `zipStart`, `zipEnd`, `zipStep` (the branch in `updatePlayer`'s movement ladder, sim.js), `zipMount` (the nearest point of a cable whose ground a body can stand on - the walk's goal, and a bot's), `zipWalkStart`/`zipWalkEnd`/`zipWalkStep` (the walk to the cable - a channel holding the stick at the top of `updatePlayer`'s input read, ended by any input of the player's own) | `zipline` (the pylon's `OBJECTS` entry: `world`; pixels: js/draw/zipline.js; the cap: `drawZipHint`, js/ui/wheel.js; the E press: `keyPress`/`ckRightPress`, input.js; the gates it joins: `tryWork`/`autoWork`/`tryDodge`/`tryProne`/`stunUnit`/`rootUnit`/`netUnit`, actions.js, `tryAbility`, abilities.js, `autoFish`, tools.js, `separateUnits`, nav.js) |
| a camp: its kind's data, the fixed mirrored sites, the clearing, standing one up, stocking it, where a position is, the respawn clock its anchor wears | `CAMPS` (the hog hut's `hut` is an `OBJECTS` row with `w`/`h`; `render()`'s hut branch draws it off `HUT_FR`), `CAMP_SITES`/`CAMP_EDGE`, `campTile`, `campSites`, `clearCamp`, `placeCamps`, `spawnCampMonster`, `stockCamps`, `campPop`, `campAt`, `CAMP_HOLD`, `updateCamps` (`C.repopT`; the anchor's `site`) | `camps` (the hover bar: `drawCampClock`, js/draw/marks.js, from the den and cairn branches of `render()` off `hovO`, js/draw/render.js) |
...
| the archery targets: the perimeter track, the two lanes and the hop, the three habits, the shared face geometry, a hit, its scoring/respawn and the consecutive-hit run | `ptargets`, `addPTarget`, `agPos`, `agEdge`, `AG_RECT`/`AG_INSET`/`AG_LEN`/`AG_LANE_GAP`/`AG_SPD`/`AG_SIZE`, `agBlocked`, `ptFace`, `ptLive`, `ptHitR`, `hitPTarget` (the impact: score and shatter land the same frame), `agShatter` (the break), `agStreak` (broken by the arrow loop, sim.js), `PT_HIT_R`/`PT_RESPAWN`/`PT_POP` | `practice arena` › `the archery track` (the arrow test: `shotContacts`, sim.js; pixels: `drawAgTrack`/`drawPTarget`, js/draw/practice.js) |
| the archery round: the bell and its held difficulty wheel, the per-difficulty spawn tables, the furniture sink (bell included), the countdown, the spawner, the record | `agame` (incl. `diff`), `AG_T`/`AG_BELL`/`AG_DIFF`, `agBellNear`, `agRing`, `agEndRound`, `agSpawn`, `agStock`, `agSinkU`, `agUpdate` (the wheel: kind `'agbell'` in keydown/keyup input.js, `wheelOptions`/`renderWheel`/`runCmd` js/ui/wheel.js; the `E RING` cap: `drawBellHint`, js/ui/wheel.js; the record: `PROFILE.bestRange`, profile.js) | `practice arena` › `the archery round: one bell` (readouts: `drawAgame`/`drawAgameUI`, js/draw/practice.js; the bell's pixels: `drawAgBell`, js/draw/practice.js; the sink crop: the dummy/rack/agbell branches, js/draw/render.js) |
...
| a fish net: the lure, the catch, handing the catch over, drawing it | `nearestNet`, `angDelta`, `updateStructures` (`net` branch, structures.js), `drawNet` (js/draw/structs.js), `NET_*` (js/structures.js) | `fish` |
...
| shooting a worker bot: its hitbox, its damage, its wreck, and who it is now angry at | `ROBOT_HIT_R`/`robotHitY` (the disc the arrow loop sweeps), `hurtRobot`, `robotDies`, `b.mad` | `workers` |
...
| the eagle's merchant: climbing down at the crash, pitching its market stall beside the spur's head (`OBJECTS.stall`), felling the back woods (away from the road) into its log stock, raising up to three bot bays (barracks) there for those logs (and rebuilding them), keeping to its post at the head of the spur, and standing still to serve whoever opens its counter | `MERCH_*` (incl. `MERCH_BAY_*`), `freeTileNear`, `spawnMerchant` (called from `eagleCrash`, boot.js), `updateMerchant` (dispatched from `updateRobot` on `b.merchant`), `merchFell`, `merchBearing`, `merchBayDir`, `merchBaySlots`, `merchNextBay`, `merchBaySite`, `merchBayBlocker`, `merchBackPine`, `merchStallSite`, `pitchStall`, `b.stall`/`b.stallT`/`b.stallSite`, `b.bays`/`b.bayT`/`b.baySite`/`b.wood`, `b.avoids`, `shopServing` (js/ui/shop.js) | `merchant` |
...
| the bag rows that make tools and bits carryable at all | the `ITEMS` / `RES_COLORS` loops at the foot of the file (the tables themselves: `ITEMS`, js/player.js; `RES_COLORS`, js/structures.js) | `tools & bits` › `icons` |
...
| what a press actually fires, and the shot it puts in the air; the fish taken with no press at all | `fireTool`, `emitBit`, `offBy`/`SHOT_SKEW`/`DUP_SKEW`/`FAN_SPREAD` and `armOff` (the volley's spread; the pierce lays its arms out through it too), `autoFish`, `FISH_AUTO_CD` | `tools & bits` › `what a tool fires` |
...
| a weapon's art at its real length in the hand, where it differs from the bag icon; which way every art's business end is drawn | `TOOL_HELD_ART` (baked as `toolHeld_<art>_<tier>`), `TOOL_FWD` | `tools & bits` › `icons` |
...
| **a found bit arming itself**: the free cells of the tool in hand counted as room, and the pickup that fills them before the pack (a bot is left to `botFitLoadout`) | `autoFitTools`, `fitRoom`, `fitAdd`, and `fitBit` — the cell a bit lands in, a shot last and a fitting ahead of every shot (also `sendBagCell`'s click, js/ui/strip.js) (`fitAdd`'s callers: the drop pickup in `updatePlay`, sim.js; `shopBuy`, js/ui/shop.js) | `tools & bits` › `a find arms itself` |
| **a discarded weapon shedding its build** as it lands — every ground-drop path calls it | `SHED_KICK`, `shedBits` (its one caller: `throwCell`, js/ui/bag.js) | `tools & bits` › `a tool that lands in the snow arrives bare` |
...
| the options on a key: the warrior's other four (WAR CRY, WHIRLWIND, HAMSTRING, SECOND WIND) and their tuning, what a key IS on a body, the key an id stands on | `CLASS_AB_ALT`, `abOptions`, `abOf`, `abKeyOf`, `CRY_*`/`WHIRL_*`/`HAM_*`/`WIND_*`, `abWarCry`, `abWhirlwind`, `abHamstring`, `abSecondWind` | `class abilities` › `the options on a key`, `warrior: the other four` (the pick: `p.abPick`, player.js; the strip: `pickAbility`, js/ui/menu.js) |
| the piercing shot wearing every modifier on the tool in hand (row order and tensile ignored), the flight that buys, and the loose | `pierceMods`, `PIERCE_FLIGHT`/`pierceFlight`, `abPierce` | `class abilities` › `hunter: the four effects` |
...
| the wedge every blade-shaped blow sweeps and draws, and the ring: units by centre plus radius, buildings by any footprint tile | `inCone`, `unitsInCone`, `structsInCone` | not here: js/actions.js, `status effects` › `what a unit IS` (beside `unitsNear`/`structsNear`) |
| every wind-up's shape on the snow for both sides and the flash where it landed (the pierce/charge line, the stomp ring, the slam/execute wedge), and the sword's sweep over the bodies | `abFx`, `drawWedge`, `drawRing`, `teleLen`, `drawTeleLine`, `drawSlashes`, `drawAbilityGround` | `class abilities` › `drawing: the world layer` |
...
| Looking for | Start at | Banner |
| --- | --- | --- |
| the sim's cosmetics recorded for screens that do not run it: a cue or a shake at a place / for a body, the ring, its flush and its replay | `sfxAt`, `sfxFor`, `sfxOwn`, `shakeAt`, `shakeFor`, `evPush`, `evDrain`, `evPlay`, `evRecord`, `evInStep`, `EV_ANYWHERE` | `sim events` |

## js/net/net.js

| Looking for | Start at | Banner |
| --- | --- | --- |
| which role this screen plays, the transport interface and the loopback solo speaks through, who counts as a person | `NET`, `SNAP_EVERY`/`LATE_JOIN_T`/`RECONNECT_GRACE`, `isHuman`, `loopbackTransport`, `netSetup` | `net roles` |
...
| Looking for | Start at | Banner |
| --- | --- | --- |
| the match's state as one plain object: which fields are skipped, which singletons ride | `SNAP_SKIP`, `SNAP_STATE` | `snapshot` |
| network ids: a stable id per moving entity, refs as tokens, the pack and its inverse | `snapNids`, `snapMaps`, `snapNid`, `snapNidSet`, `snapKindOf`, `snapToken`/`snapDeref`, `packEnt`, `pack`/`unpack`/`snapResolve` | `snapshot` › `network ids` |
| building the full form and weighing it | `snapBuild`, `snapSize` | `snapshot` › `build` |
| writing a full form back into the world | `snapApply` | `snapshot` › `apply` |
| the ack-keyed delta: the shadow, the history ring, the cut from a base, the in-place apply | `HIST_KEEP`, `snapShadow`, `snapShadowReset`, `snapSame`, `snapFieldDiff`, `snapHistoryPush`, `snapHistoryAt`, `snapDeltaFrom`, `snapBuildDelta`, `snapTileSame`, `snapApplyDelta`, `snapAssign`/`SNAP_MERGE` | `delta` |
| the bytes: the binary form, the shared key dictionary, the quantized positions and velocities | `SNAP_Q`, `SNAP_QPOS`, `SNAP_QUANT`, `snapQable`, `snapQi`, `snapQv`, `ENC_TAG`, `encDict`, `snapEncode`, `snapDecode` | `encode` |
| the wire's proofs: the echo through the bytes, a run sent as deltas, the compare and its one tolerance | `snapBlank`, `snapFrame`, `snapCompare`, `snapCompareLoose`, `snapQuantize`, `netEcho`, `netEchoRun`, `netDeltaRun` | `the echo harness` (how to run them: [checklists](checklists.md#verifying-a-change); the design: [pvp-architecture](../pvp-architecture.md)) |

## js/net/transport-ws.js

| Looking for | Start at | Banner |
| --- | --- | --- |
| the WebSocket transport over the relay, and the live list of open rooms | `WS_RETRY`, `wsUrl`, `wsTransport`, `wsRooms` | `ws transport` (the relay itself: app/server.js) |

## js/net/transport-steam.js
...
| which body a bit flies as, and the four that are not the arrow | `BIT_BODY`, `drawTumbler`, `drawMote`, `drawSwungBody`/`FIST_MAP`/`AXE_MAP`/`BIT_INK`, `drawWarpShot` | `render` (`drawTumbler`, `drawMote`, after the shots pass) and `render` › `the swung bodies` (the rest, with `BIT_BODY` at its foot; the names they answer to: `body` on `BITS`, tools.js) |
...
| the ring and the risen icon a TOOL SWAP raises, on every player | `drawSwaps` | `render` › `the teleport's flash` (below `drawWarps`; the event itself: `swapFx`/`swaps`, tools.js) |
...
| the HUD SIZE scale (`settings.hudScale`): the scale capped where the strip would outgrow the view, and the pointer's map back through the strip's bottom-centre anchor | `hudSc`, `stripMouse` | `the hud frame` (the bakes and blits: `drawHudScaled`/`drawCornerScaled`, js/ui/hud-draw.js) |
...
| the hatch and cut corners that mark a MODIFIER bit apart from a projectile, in every well either sits in | `modPlate` (its callers: `drawBag` js/ui/bag.js, `drawShelf`/`drawDragGhost` here, `drawTooltip` js/ui/tooltip.js, `drawShopWell` js/ui/shop.js, `drawWikiRow` js/ui/menu.js, `drawToolPrimer` js/ui/panels.js) | `the plates, bars and icons the widgets share` (the `proj` flag it reads: `BITS`, tools.js) |
...
| the HUD SIZE scale: the 1x bake blitted about the strip's bottom-centre anchor, and the top-left corner's own bake (the shelf, the drawer, the hammer plate and the build list: `drawCorner`) | `hudScaleCv`, `drawHudScaled`, `cornerScaleCv`, `drawCorner`, `drawCornerScaled`, `HUD_BAKE_HEAD` (the scale and the pointer's map back: `hudSc`/`stripMouse`, js/ui/strip.js) | `drawing the strip, the shelf and the carried item` |
...
| Looking for | Start at | Banner |
| --- | --- | --- |
| the frame's UI pass in order - what goes over the world while you play, from the minimap to the day headline | `renderUI`, `dayPopCv` (the day headline's bake) | `renderUI` (last of the HUD files; the pieces it calls live in the seven files above it) |
...
| a price move worth telling everyone about | `marketNews` (the good's own `news` gold floor beside `MKT_NEWS`) | `market` (the log line: `logEvent`'s colour override, js/ui/panels.js; the plate: `raiseNotice`, below; the cue: `SFX.market`, audio.js) |
| the plates top-right under the minimap - the market's, the roost's and the stat sheet's: raising one, ageing it, where it sits, its pixels | `NOTE_MAX`/`NOTE_LIFE`/`NOTE_IN`/`NOTE_FLASH`/`NOTE_WASH`/`NOTE_OUT`/`NOTE_SLIDE`/`NOTE_FR`/`NOTE_W`/`NOTE_H`/`NOTE_AIR`/`NOTE_GAP`, `NOTE_KIND` (the palette the feed line shares) + `NOTE_MARKS` (a STAMPED mark, named by a kind's `glyph` - the roost's bird), `raiseNotice`, `ageNotices` (called from `updateFx`, sim.js), `noteRect`, `renderNotices` (called from `renderUI`, js/ui/compose.js) | `notices` |
| **a plate is its kind's size**, and the lane sums the heights it finds rather than counting a pitch: the per-kind width, height and body, which slot holds which kind, and where the lane stops rather than run under the hud strip | `noteW`/`noteH`/`notePitch`, `noteKindAt`, `noteLaneFloor`, `noteBody` (the default mark / headline / tail row a kind with no `body` of its own wears), `noteCard`/`noteBox`/`noteFrame` (all sized by the kind) | `notices` |
| **the stat ledger**: your whole sheet flown into that lane when a number on it moves, the moved rows lit and blinking, one plate up at a time | `STAT_PAD`/`STAT_TOP`/`STAT_PITCH`/`STAT_PULSE`/`STAT_W`, `STAT_LABEL`/`STAT_VAL`/`STAT_LIT`/`STAT_DOT`/`STAT_UP`/`STAT_DOWN`, `statPlateH` (the kind's `h` getter - `GEAR_STATS` loads after this file), `statSeen`/`statOwner`/`statLit`, `raiseStatNote`, `updateStatLedger` (called from `updateFx`, sim.js), `statDeltaTxt`, `noteStatBody` | `the stat ledger: your sheet, as a notice` (the table it reads: `GEAR_STATS`, js/ui/menu.js; the cue: `SFX.stat`, audio.js) |
| what makes a plate read as a stamped tag: the shadow, the notched corners, the bevel and the life draining along its base | `noteCard`, `noteBox` (the notched rect), `noteFrame` | `notices` |
| the plates' own art: the two 16x16 marks (a price's sack, a turnover's crate), the STAMPED one (the roost's bird, a `NOTE_MARKS` grid inked in your side's colour) and the two tails | `NOTE_KIND[k].mark`/`.glyph` picks it - `SPRITES.goldSack` (ten 16x16 grids, `sack`/`SACKPAL` in the `gold sack` section of js/sprites/items.js, resampled off `docs/media/new_media3/bag_of_gold_bouncing.png`) or `SPRITES.crate` (the 16x16 `crate`/`CRATE_PAL` in the `crate` section) - and `NOTE_TAILS` (up, down; a kind may have none) | `notices` (the tails stamped with `stampGrid`: `the art`, js/ui/screens.js) |
| the twelve offers (one item each: a bought one is `null` until the turnover) and their turnover | `SHOP_RESTOCK`, `SHOP_COLS`, `SHOP_SECTIONS`, `SHOP_CARD_ODDS`, `shopPick`, `shopRestock`, `shopOffer` | `the counter's stock` |
...
| standing at a counter (a merchant or its stall), and the merchant standing still to serve it | `SHOP_REACH`, `counterPt`, `stallUp`, `merchNear`, `inReach`, `shopServing` (read by `updateMerchant`, robots.js) | `buying and selling` |
| the trades | `shopBuy` (contested on its well; `shopCanBuy` is the check it runs on the click and again in the claim), `shopSell`, `shopSellCell`, `shopTrade`, `shopCmd` (the `runCmd` entry, js/ui/wheel.js), `shopFx`, `shopDeny`/`shopNoRoom` | `buying and selling` (a sale pays through `tradeGold`, player.js - gold without XP) |
...
| what is in the weapon well and the pile, and filling them from the pack or the shelf | `state.forgeSel` (a pointer to the weapon's cell) + `forgeFind`, `state.forgePile` + `forgePileNow`/`pileCount`, `forgeSelect`, `forgeAdd`, `forgePut` (also from `sendBagCell`/`sendSlot`, js/ui/strip.js), `forgeCarried`, `forgeReset` (from `closeShop`), `FORGE_ORES` | `the forge tab` |
| drawing it - the wells, the forge bar, the preview, the plate, the ore plates - and the `+N` a forged weapon wears in every well | `drawForge` (from `drawShopPanel`), `forgeWell`, `forgeGhost`, `forgeArrow`, `forgeUpArrow`, `FORGE_WELL`/`FORGE_KIND_H`/`FORGE_INK`, `forgeMark` (also `drawBag`, js/ui/bag.js; `drawShelf`, js/ui/hud-draw.js) | `the forge tab` |
...
| Looking for | Start at | Banner |
| --- | --- | --- |
| the title screen: the words, panels, play intro | `MENU_TXT_SCALE`/`MENU_TXT_PITCH`/`MENU_BOTTOM`, `menuLayout`, `drawTitleBackdrop`, `beginIntro`, `renderTitle` (the frost plank other screens stand: `drawMenuButton`, `drawMenuSlab`) | `main menu` |
| the seed die on the lobby (under the map), and the reroll | `drawSeedDie`, `rerollWorld`, `lobbyLayout().seed` | `main menu` › `lobby` |
| lobby: the painted night, the two team panels glued to the screen's edges (your side left, rivals right, face-down until the count), the map plate and the target side by side at the top centre with their names in white under them (each the way into its pop-up), your character on stage at 3x with a chevron either side that swaps to the neighbouring character, the gear row under its feet, LOCK IN as a bare word at the foot and the five-second count that takes its place (a second press skips it); the three pop-ups' shared ease, slab, X and inert hits | `LOBBY_*`, `lobbyLayout`, `lobbyHit`, `CLASS32`/`classIcon32` (and the rail's 12px `CLASS12`/`classIcon12` beside them), `drawLobbyBackdrop`, `drawLobbyStage`, `drawLobbyCard`, `drawLobbyRosters`, `pxDisc`, `drawLobbyTarget`/`wreckTargetFace`, `drawDiffPlates`, `drawChevron`, `drawMapPlate`, `drawLobbyTop`, `drawLobbyCount`, `renderLobby`, `lobbySlot`/`lobbyStep`, `mapStep`, `pressPlay`, `cancelCount`, `lobbyRevealed`, `setAiLevel`, `lockIn`, `COUNT_T`, `LOCK_FX_T`, `popOpen`/`popHit`/`openPop`/`leavePop`, `drawPopSlab`/`drawPopX`/`popFrameHit`, `POP_PIC` | `main menu` › `lobby` (the levels' names: `AI_LEVELS`, ai.js; the slots' store: `PROFILE`, profile.js; the shape and the page: `mapChip`/`pickMap`, `the map`) |
| the AI pop-up over the lobby: the target big beside the three difficulty plates | `aiLayout`, `aiScreenHit`, `beginAiPick`, `aiKey`, `aiClick`, `renderAiPick` | `main menu` › `the AI pop-up` |
| the map pop-up over the lobby: the picture with its chevrons, the shape's name, the seed row and the die | `mapLayout`, `mapScreenHit`, `beginMapPick`, `mapKey`, `mapClick`, `renderMapPick` | `main menu` › `the map pop-up` |
| entering/leaving the practice arena | `beginPractice`, `leavePractice` | `main menu` |
| the patch tag bottom-right of the title and the list it prints | `PATCH_TXT`, `PATCH_NOTES`, `patchTagRect`/`overPatchTag` | `main menu` |
| the patch notes screen: the hand-written digest, every patch folded by tenth, what is open, the layout, what is under a point, the keys, the WIKI word, the draw | `PATCH_DIGEST`, `DIGEST_FROM`, `NOTES_GROUPS`, `notesOpen`, `notesScroll`, `NOTES_W_MAX`/`NOTES_TOP`, `notesLayout`, `notesHit`, `beginNotes`/`leaveNotes`, `notesScrollBy`, `notesToggle`, `notesKey`, `notesClick`, `drawNotesChevron`, `renderNotes` | `patch notes` (its ease: `menu.notesT`, core.js; the wheel: input.js; the cursor: `cursorInfo`, js/draw/render.js) |
| the rooms screen: the open rooms (the relay's, or Steam's lobbies) as planks under HOST, hosting and joining one, the waiting room's plates and pips, the in-match link pip | `RM_*`, `roomsFeed`, `beginRooms`, `leaveRooms`, `roomsLayout`, `roomOk`, `roomsHit`, `hostRoom`, `joinRoom`, `roomsKey`, `roomsClick`, `updateRooms`, `drawRelayPip`, `drawSeatPips`, `drawCodePlate`, `renderRooms`, `drawRoomPlate`, `drawNetLink` | `rooms` (its readers in the lobby: `updateTitle`, `drawLobbyCard`, `renderLobby`, further down the file; the doors it opens: `netHost`/`netJoin`/`netLeave`, js/net/net.js) |
| the pre-match build: the hero pop-up off your figure on the lobby - live preview, stat ledger with hover deltas, the gear column and the ability column (a well a piece, a well a key, a click turning each, a hover previewing the swap in the ledger), the equip flash, the stat points spent on the ledger's own rows (the unspent pips, a row's gold pips that give a point back), the ability card on hover | `heroLayout`, `heroScreenHit`, `pickGear`, `pickAbility`, `spendPt`, `renderHero`, `drawGearWell`, `drawGearPreview`, `heroPreviewKit`, `GEAR32`/`gearIcon32`, `beginHero`/`leaveHero`, `heroPipX`, `HERO_*` | `main menu` › `the hero pop-up` (the numbers' base: `baseKit`; the tracks and the budget: `STAT_TRACKS`/`STAT_POINTS`/`ptsSpent`, player.js `stat points`) |
| **what a stat IS**: the one table the hero pop-up, the character panel and the HUD's stat ledger all price a body with - label, getter `(kit, player?)`, format, and which way is better | `GEAR_STATS`, `GS_INT`/`GS_SEC`/`GS_PCT`/`GS_NUM`/`GS_LV` (the level's hp and damage, which the kit itself never holds - `levelMaxHp`, player.js and `emitBit`, tools.js) | `main menu` › `the gear pop-up` (its three readers: `renderGear` here, `drawCharPanel` js/ui/bag.js, `noteStatBody` js/ui/shop.js) |
| the wiki: its pages as data, the slab, tabs, window and rail, the per-page scroll, what is under a point, the way in and out | `WIKI_PAGES`, `WIKI_W_MAX`/`WIKI_H`, `wikiScroll`, `wikiBlocks`, `wikiPage`, `wikiLayout`, `wikiHit`, `beginWiki`/`leaveWiki` (back to `menu.wikiFrom`), `wikiScrollBy`, `wikiSetTab`, `wikiKey`, `wikiClick`, `renderWiki`, `drawWikiRail` | `main menu` › `the wiki` (the tooltip a row raises: `tipKind`, js/ui/tooltip.js; the wheel: input.js; its song WHISPERING WOODS: `TRACKS.wiki`, audio.js) |
...
| the WORLD page: the setting's ten entries (name, paragraph, the sprites on its mound), the figure column, the mound the beasts share, the `lore` draw in `renderWiki` | `WIKI_WORLD`, `WIKI_FIG_W`/`WIKI_TEXT_X`, `drawWikiFig`, `wikiMound` | `main menu` › `the wiki` (the premise, the fixed words and the voice: [lore.md](lore.md); the road's colours: `ROAD_COL_A`/`ROAD_COL_RUT`, js/draw/ground.js) |
| the CLASSES page: an ability row's columns, the four stat pips, the word-wrap the blurbs and the WORLD page use, the `cls` and `ab` draws in `renderWiki` | `WIKI_AB_COLS`, `WIKI_STATS`, `wikiWrap` | `main menu` › `the wiki` (the classes and kits: `CLASSES`, player.js; the abilities: `CLASS_AB`, `classAbIcon`, abilities.js; the card a hover raises: `tipClassAb`, js/ui/tooltip.js) |
