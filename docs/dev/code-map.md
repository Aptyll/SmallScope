...
| Looking for | Start at | Banner |
| --- | --- | --- |
| the title screen: the words, panels, play intro | `MENU_TXT_PITCH`/`MENU_BOTTOM`, `menuLayout`, `drawTitleBackdrop`, `beginIntro`, `renderTitle` (the frost plank other screens stand: `drawMenuButton`, `drawMenuSlab`) | `main menu` |
| the seed die on the map pop-up, and the reroll | `drawSeedDie`, `rerollWorld`, `mapLayout().seed` | `main menu` › `the map pop-up` (the landing: `softfall.map`, js/boot.js) |
...
| the practice item's breakable ice, and entering/leaving the arena | `menuFrozen`, `iceRefuse`, `breakPracticeIce`, `beginPractice`, `leavePractice` | `main menu` (the knocks so far: `menu.iceMarks`) |
...
| the DOWNLOAD tag, bottom centre of the title in a browser | `DOWNLOAD_URL`, `IS_APP`, `downloadTagRect`, `overDownloadTag`, `drawDownloadTag` | `main menu` (just above `rooms`) |
