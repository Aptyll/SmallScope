...
| the pre-match build: the hero pop-up off your figure on the lobby - live preview, stat ledger with hover deltas, the gear strip and the ability strip (a well a piece, a well a key, chevrons above and below that turn them), the equip flash, the stat points' pips and +/- plates, the ability card on hover | `heroLayout`, `heroScreenHit`, `pickGear`, `pickAbility`, `spendPt`, `renderHero`, `drawGearWell`, `drawGearPreview`, `drawChevronV`, `heroPreviewKit`, `GEAR32`/`gearIcon32`, `beginHero`/`leaveHero`, `HERO_*` | `main menu` › `the hero pop-up` (the numbers' base: `baseKit`; the tracks and the budget: `STAT_TRACKS`/`STAT_POINTS`/`ptsSpent`, player.js `stat points`) |
