...
| picking variants pre-match: the pop-up over the lobby - live preview, stat ledger with hover deltas, twelve 32×32 icon wells, the equip flash | `gearLayout`, `gearScreenHit`, `pickGear`, `renderGear`, `drawGearWell`, `drawGearPreview`, `gearPreviewKit`, `GEAR32`/`gearIcon32`, `beginGear`/`leaveGear` | `main menu` › `the gear pop-up` (the numbers' base: `baseKit`, player.js) |
| **what a stat IS**: the one table the gear pop-up, the character panel and the HUD's stat ledger all price a body with - label, getter `(kit, player?)`, format, and which way is better | `GEAR_STATS`, `GS_INT`/`GS_SEC`/`GS_PCT`/`GS_NUM`/`GS_LV` (the level's hp and damage, which the kit itself never holds - `levelMaxHp`, player.js and `emitBit`, tools.js) | `main menu` › `the gear pop-up` (its three readers: `renderGear` here, `drawCharPanel` js/ui/bag.js, `noteStatBody` js/ui/shop.js) |
