...
| the bytes: any snapshot-shaped value as binary, object keys as indices into a dictionary both ends grow in step (a message leads with the names it is the first to use; a numeric key goes as a number), numbers as the smallest integer or a float32 | `ENC_TAG`, `encDict`, `snapEncode`, `snapDecode` | `encode` (js/net/snapshot.js) |
| the wire's proofs: the echo through the bytes, a run sent as deltas and checked after each, a client checking itself against the host's full form mid-match | `netEcho`, `netEchoRun`, `netDeltaRun`, `snapCompare`, `snapCompareLoose`, `NET.verify`/`VERIFY_EVERY` | `the echo harness` (js/net/snapshot.js), `client` (js/net/net.js) |
