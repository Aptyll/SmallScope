...
| the hud strip (bottom-centre): five wells - the weapon first, the four ability wells (a pip per point, 2x key digit, dim all over while the key is locked) following, the two stacked meal buttons (Q/F key, icon, `shortNum` count, the shared food clock, the refusal flash) and the always-on PURSE tab standing on the rim over them, on the right end - over the segmented plum xp bar, with the floating buy plates bobbing above affordable wells while the strip is home (`hudInT`/`hudHome`) | `AB_CELL`/`FOOD_CELL`/`FOOD_W`/`FOOD_ICON_X`/`PURSE_H`/`FOOD_BTNS`/`AB_SEGS`/`AB_BUY`/`LOCK_DIM`/`HUD_SLIDE`/`hudStripRect`/`hudInT`/`hudHome`/`stripCellRect`/`toolCellRect`/`abCellRect`/`foodCellRect`/`pursePlateRect`/`foodDenied`/`toolDenied`/`abDenied`/`abBuyRect`/`abBuyHit`/`stripHit`/`drawToolCell`/`drawClassAbCell`/`drawFoodCell`/`drawPurse`/`drawAbBuyPlate`/`drawXpBar`/`drawHudStrip` | `UI` › `hud strip` |
