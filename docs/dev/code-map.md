...
| the plates top-right under the minimap - the market's three and the roost's: raising one, ageing it, where it sits, its pixels | `NOTE_MAX`/`NOTE_LIFE`/`NOTE_IN`/`NOTE_FLASH`/`NOTE_OUT`/`NOTE_SLIDE`/`NOTE_FR`/`NOTE_W`/`NOTE_H`/`NOTE_PITCH`/`NOTE_GAP`, `NOTE_KIND` (the palette the feed line shares) + `NOTE_GLYPH` (a DRAWN mark by name - the roost's bird diamond), `raiseNotice`, `ageNotices` (called from `updateFx`, sim.js), `noteRect`, `renderNotices` (called from `renderUI`, js/ui/compose.js) | `notices` |
...
| a client's half: its input out each tick with the edges cleared and the newest tick it applied as its ack, welcome/full/snap/refuse in (a snapshot no newer than the one held ignored, a delta cut from a base newer than what is held refused with a resync), the roster built with its own slot the human, the host's verdict kept out of ours, and the screen the local body calls for (ride, death, play, the end) | `netClientStep`, `netClientHello`, `netClientWelcome`, `netClientApply`, `netClientMode` | `client` (js/net/net.js) |
| the WebSocket transport over the relay: a host given a code by the greeting, a client joining by one, the room's listing data, the live list of open rooms | `WS_RETRY`, `wsUrl`, `wsTransport`, `wsRooms` | `ws transport` (js/net/transport-ws.js) |
| which relay this screen talks to, whether this screen rides Steam, the three doors the title opens (host, join, leave), a room's listing and the roster to every peer | `RELAY_DEFAULT`, `netRelay`, `netSteam`, `netTransportFor`, `netHost`, `netJoin`, `netLeave`, `netHostRoom`, `netHostRoster`, `netClientRoster` | `net roles` (js/net/net.js) |
| the rooms screen: the relay's open rooms as planks under HOST (a relay pip, a ghost row while empty; each room its host's name, ten seat pips five a side, its code on a plate, a red dot once live, another patch dimmed with its patch printed), a join that reloads onto the room's seed, the waiting room it opens with the room's code plate, the crown and the person rims on the cards, the guest's frozen host-named plank, the join/leave flash, a guest sent back here when its host leaves; Steam's lobbies in the list under the wrapper; the in-match link pip | `RM_*`, `roomsFeed`, `beginRooms`, `leaveRooms`, `roomsLayout`, `roomOk`, `roomsHit`, `hostRoom`, `joinRoom`, `roomsKey`, `roomsClick`, `updateRooms`, `drawRelayPip`, `drawSeatPips`, `drawCodePlate`, `renderRooms`, `drawRoomPlate`, `drawNetLink` | `rooms` (js/ui/menu.js), `updateTitle`/`drawSelectCard`/`renderSelect` (js/ui/menu.js) |
| the DOWNLOAD tag, bottom centre of the title in a browser | `DOWNLOAD_URL`, `IS_APP`, `downloadTagRect`, `overDownloadTag`, `drawDownloadTag` | `title` (js/ui/menu.js) |
| the Steam transport under the wrapper: the lobby made or joined, its owner the host, chat updates as peer/gone, text as JSON on the reliable channel (parts over `STEAM_CHUNK`), the wire form's bytes raw in binary frames - a lossy delta split into `STEAM_UNREL`-byte parts on the UNRELIABLE channel and dropped whole when one never comes (`lost`), everything else reliable - the lobby's data as the room's listing (roomData), and the rooms screen's feed of lobbies | `STEAM_LIST_T`, `steamRooms`, `STEAM_CHUNK`, `STEAM_UNREL`, `STEAM_LOSSY_PARTS`, `STEAM_FRAME_HDR`, `STEAM_MAGIC`, `STEAM_PENDING`, `STEAM_LOBBY_MAX`, `steamFrames`, `steamTransport` (`send`, `receive`, `receiveFrame`, `roomData`) | `steam transport` (js/net/transport-steam.js) |
| network ids: a stable id per moving entity, the maps a token resolves through on either side | `snapNids`, `snapMaps`, `snapNid`, `snapNidSet`, `snapKindOf`, `packEnt` | `snapshot` › `network ids` (js/net/snapshot.js) |
| the delta, ack-keyed: a ring of what the world packed at each flush tick and what that tick touched (per entity the names that changed or left, the ids gone, the order, the eagles, the singletons, each tile's touched names, the ground cells), and the cut from any base in the ring as the UNION of the entries after it (a field that flipped and flipped back still goes); the apply that updates entities in place and reports what moved | `HIST_KEEP`, `snapShadow`, `snapShadowReset`, `snapSame`, `snapFieldDiff`, `snapHistoryPush`, `snapHistoryAt`, `snapDeltaFrom`, `snapBuildDelta`, `snapTileSame`, `snapApplyDelta`, `snapAssign`/`SNAP_MERGE` | `delta` (js/net/snapshot.js) |
| the bytes: any snapshot-shaped value as binary, object keys as indices into a dictionary both ends grow in step (a message's header carries every name from the index the receiver's acked message left the list at, written by position; a numeric key goes as a number), numbers as the smallest integer or a float32, a position or a velocity (by field name: SNAP_QUANT) as an int16 count of eighths of a px - floored for x/y, rounded for the velocities - and the shadow compare reading the same quantized value | `SNAP_Q`, `SNAP_QPOS`, `SNAP_QUANT`, `snapQable`, `snapQi`, `snapQv`, `ENC_TAG`, `encDict`, `snapEncode`, `snapDecode` | `encode` (js/net/snapshot.js) |
| the wire's proofs: the echo through the bytes, a run sent as deltas and checked after each (with a share thrown away unacked, so cuts come from older bases), a client checking itself against the host's full form mid-match (each axis of an easing body at its own target) under a host that throws away a share of its sends; the compare's one tolerance (a quantum on the named fields) and the wire's designed loss written into the world before the echo's reference frame | `netEcho`, `netEchoRun`, `netDeltaRun`, `snapQuantize`, `snapCompare`, `snapCompareLoose`, `NET.verify`/`VERIFY_EVERY`, `NET.lossOut` | `the echo harness` (js/net/snapshot.js), `client` (js/net/net.js) |
| a client easing every moved body from where it is drawn to where the host put it over one snapshot interval | `LERP_SNAP`, `netClientApplyDelta`, `netClientLerp` | `client` (js/net/net.js) |
| the match's state as one plain object: reflection over every entity with refs as tokens, the apply that writes it back (players and eagles field by field, every other array wholesale, the road registry merged by identity, the ground repainted where it changed), the echo harness and its field-level audit | `SNAP_SKIP`, `SNAP_STATE`, `snapToken`/`snapDeref`, `pack`/`unpack`/`snapResolve`, `snapBuild`, `snapSize`, `snapApply`, `snapBlank`, `snapFrame`, `snapCompare`, `netEcho`, `netEchoRun` | `snapshot` (js/net/snapshot.js) |
...
| the fixed step: the rAF loop banks each frame's time and steps the sim in whole 1/60 s slices (up to three a frame, the rest dropped; a quarter-slice of slack so a 60 Hz screen steps once a frame), polls the pad and the fingers once a frame before them, renders once after | `TICK_DT`, `TICK_MAX`, `TICK_SLACK`, `tickAcc`, `loop` (`padPoll`, `touchPoll`) | `the fixed step` |
