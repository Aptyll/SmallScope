...
| **a found bit arming itself**: the free cells of the tool in hand counted as room, and the pickup that fills them before the pack (a bot is left to `botFitLoadout`) | `autoFitTools`, `fitRoom`, `fitAdd` (its callers: the drop pickup in `updatePlay`, sim.js; `shopBuy`, js/ui/shop.js) | `tools & bits` › `a find arms itself` |
