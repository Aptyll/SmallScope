...
| the eagle's merchant: climbing down at the crash, felling the back woods (away from the road) into its log stock, raising up to three bot bays (barracks) there for those logs (and rebuilding them), keeping to its post at the head of the spur, and standing still to serve whoever opens its counter | `MERCH_*` (incl. `MERCH_BAY_*`), `freeTileNear`, `spawnMerchant` (called from `eagleCrash`, boot.js), `updateMerchant` (dispatched from `updateRobot` on `b.merchant`), `merchFell`, `merchBearing`, `merchBayDir`, `merchBaySlots`, `merchNextBay`, `merchBaySite`, `merchBayBlocker`, `merchBackPine`, `b.bays`/`b.bayT`/`b.baySite`/`b.wood`, `b.avoids`, `shopServing` (js/ui/shop.js) | `merchant` |
