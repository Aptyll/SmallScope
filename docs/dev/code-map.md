...
| the road: its width and ragged edge, where it runs, its geometry on the diagonal, laying it (posts, pennants, the centre marker), how far the ice keeps off it, the march's waypoints | `ROAD_HW`/`ROAD_RAG`/`ROAD_RUT`/`ROAD_SHOULDER`/`ROAD_ICE_KEEP`/`ROAD_ICE_TAPER`/`ROAD_STEP`/`ROAD_POST_STEP`/`ROAD_POST_OUT`, `roadSpan`, `roadAlong`/`roadOffS`/`roadOff`/`roadPoint`, `roadEdgeAt`, `roadDist`, `onRoad`, `placeRoad` (called from `startGame`, boot.js), `roadWaypoints` | `world` › `the road` (pixels: `paintRoadOverlay` + `ROAD_COL_*`, `POST_SPR`/`CAIRN_SPR`, `bannerCloth`, draw-world.js; the maps: `updateMinimap`, ui.js, `buildWorldMapImg`, panels.js) |
...
| the backpack (bottom-right, ALWAYS up, no button): the frame's ten-cell grid, the refusal flash, the full-bag amber rim | `HUD_CELL` (the one well size the strip, the pack and the shelf share), `BAG_CELL`/`BAG_GAP`/`BAG_PAD`/`BAG_BG`/`BAG_WELL`, `bagFrameRect`, `bagCellRect`, `bagCellPlate`, `cornerMouse`/`cornerToScreen` (the HUD SIZE map about the bottom-right corner), `bagHit`, `bagClick`, `bagDenied`, `drawFoodClock`, `drawBag` | `UI` › `the backpack` |
...
| the hud strip (bottom-centre): five wells - the weapon first, the four ability wells (a pip per point, 2x key digit, dim all over while the key is locked) following, the pouch block on the right end - berry over fish, gold over cards, each a key cap, a square icon plate and a `shortNum` count, the meals under the shared food clock, the buttons' refusal flash - over the segmented plum xp bar, with the floating buy plates bobbing above affordable wells while the strip is home (`hudInT`/`hudHome`) | `AB_CELL`/`FOOD_CELL`/`FOOD_KEY_W`/`FOOD_SQ`/`FOOD_W`/`POUCH_W`/`FOOD_BTNS`/`AB_SEGS`/`AB_BUY`/`LOCK_DIM`/`HUD_SLIDE`/`hudStripRect`/`hudInT`/`hudHome`/`stripCellRect`/`toolCellRect`/`abCellRect`/`pouchCellRect`/`foodCellRect`/`goldCellRect`/`foodDenied`/`cardDenied`/`cardTotal`/`cardFanCv`/`toolDenied`/`abDenied`/`abBuyRect`/`abBuyHit`/`stripHit`/`drawToolCell`/`drawClassAbCell`/`drawPouchCell`/`drawFoodCell`/`drawGoldCell`/`drawAbBuyPlate`/`drawXpBar`/`drawHudStrip` | `UI` › `hud strip` |
| the cooldown sweep: League's radial clock cut to a square, and the ONE shape every wait is drawn in - the weapon well, the four ability wells and both meal buttons (through `drawFoodClock`); rasterised a pixel at a time and coalesced into one fillRect a run, the hand dropped below 12px of cell | `drawSweepCover`/`CD_SWEEP`/`CD_EDGE` | `UI` › `hud strip` |
| the HUD SIZE scale: the 1x bake blitted about the strip's anchor (bottom-centre, stepping left of the corner when the dial grows the two into each other), the pointer's map back through it, and the corner's own bake about the bottom-right | `hudSc`, `stripAnchorX`, `CORNER_REACH`, `stripMouse`, `hudScaleCv`, `drawHudScaled`, `cornerScaleCv`, `drawCornerScaled` (the slider: `settings.hudScale`, set in `applySliderDrag`, panels.js) | `UI` › `hud strip` / `the weapon shelf` |
...
| the WEAPON SHELF on the pack's top edge, up at all times: the row (cell -1 is the tool), what the pointer is on, the rails that say which fitting reaches which shot, one well, the draw | `SHELF_CELL`/`SHELF_GAP`/`SHELF_BAR`/`SHELF_RAIL`/`SHELF_SLOT`, `shelfCell`, `shelfUp`, `shelfCellRect`, `shelfHit`, `shelfRails`, `shelfTopY`, `shelfWell`, `drawShelf` | `UI` › `the weapon shelf` |
...
| the "!" a tool wears when its build weighs more than one press can swing | `drawOverWarn` (from `drawToolCell`, `drawShelf` and the CONTROLS page's `drawToolPrimer`, panels.js; the answer it draws: `toolOver`, tools.js) | `UI` › `hud strip` |
