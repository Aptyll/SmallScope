...
| the per-tick snapshot form: the moving arrays in full, tiles and ground only where they changed against a shadow of the last send, every building every tick | `snapShadow`, `snapShadowReset`, `snapObjSame`, `snapBuildTick` | `snapshot` › `the per-tick form` (js/net/snapshot.js) |
