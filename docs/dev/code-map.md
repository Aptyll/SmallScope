...
| a host's half of the match: peers' inputs merged into their bodies (held fields overwrite, edges latch), a HELLO into the smaller side's first AI slot or its parked one back, a vanished peer into a bot, the roster, the tick snapshot and the cosmetics out every SNAP_EVERY ticks | `netHostStep`, `netHostHello`, `netHostLeave`, `netRoster`, `netHostFlush` | `host` (js/net/net.js) |
| a client's half: its input out each tick with the edges cleared, welcome/full/snap/refuse in, the roster built with its own slot the human, the host's verdict kept out of ours, and the screen the local body calls for (ride, death, play, the end) | `netClientStep`, `netClientHello`, `netClientWelcome`, `netClientApply`, `netClientMode` | `client` (js/net/net.js) |
...
| the delta: per entity the fields that changed since the last send (and the names that left as $del), the arrays' order when it changed, departures, tiles and ground where they changed (a tile with only primitive fields compared without packing), each singleton field by field; the apply that updates entities in place and reports what moved | `snapShadow`, `snapShadowReset`, `snapSame`, `snapFieldDiff`, `snapKindDelta`, `snapBuildDelta`, `snapTileSame`, `snapObjSame`, `snapApplyDelta`, `snapAssign`/`SNAP_MERGE` | `delta` (js/net/snapshot.js) |
| the bytes: any snapshot-shaped value as binary, object keys as indices into a dictionary both ends grow in step (a message leads with the names it is the first to use; a numeric key goes as a number), numbers as the smallest integer or a float32, a position or a velocity (by field name: SNAP_QUANT) as an int16 count of eighths of a px - floored for x/y, rounded for the velocities - and the shadow compare reading the same quantized value | `SNAP_Q`, `SNAP_QPOS`, `SNAP_QUANT`, `snapQable`, `snapQi`, `snapQv`, `ENC_TAG`, `encDict`, `snapEncode`, `snapDecode` | `encode` (js/net/snapshot.js) |
| the wire's proofs: the echo through the bytes, a run sent as deltas and checked after each, a client checking itself against the host's full form mid-match (each axis of an easing body at its own target); the compare's one tolerance (a quantum on the named fields) and the wire's designed loss written into the world before the echo's reference frame | `netEcho`, `netEchoRun`, `netDeltaRun`, `snapQuantize`, `snapCompare`, `snapCompareLoose`, `NET.verify`/`VERIFY_EVERY` | `the echo harness` (js/net/snapshot.js), `client` (js/net/net.js) |
