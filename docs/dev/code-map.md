...
| the slab's foot planks: CLOSE, and in a match LEAVE PRACTICE or LEAVE MATCH beside it | `SET_FOOT_Y`/`SET_PLANK_*`, `footPlanks`, `leavePlankRect`, `settingsClose` (the clicks: `closeMenuPanel`/`leavePractice`, menu.js; `toLobby`, screens.js) | `settings menu (ESC)` |
