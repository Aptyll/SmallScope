...
| lobby: the painted night, the two team panels glued to the screen's edges (your side left, rivals right, face-down until the count), the rivals' three difficulty plates and the target whose arrows fly in per level, the map at the top centre with its chevrons and the seed row, your character on stage at 3x with a chevron either side that swaps to the neighbouring character, the gear row under its feet, LOCK IN as a bare word at the foot and the five-second count that takes its place (a second press skips it) | `LOBBY_*`, `lobbyLayout`, `lobbyHit`, `CLASS32`/`classIcon32` (and the rail's 12px `CLASS12`/`classIcon12` beside them), `drawLobbyBackdrop`, `drawLobbyStage`, `drawLobbyCard`, `drawLobbyRosters`, `pxDisc`, `drawLobbyTarget`, `drawLobbyDiff`, `drawChevron`, `drawLobbyMap`, `drawLobbyCount`, `renderLobby`, `lobbySlot`/`lobbyStep`, `mapStep`, `pressPlay`, `cancelCount`, `lobbyRevealed`, `setAiLevel`, `lockIn`, `COUNT_T`, `LOCK_FX_T`, `TGT_R`/`TGT_ANG` | `main menu` › `lobby` (the levels' names: `AI_LEVELS`, ai.js; the slots' store: `PROFILE`, profile.js; the shape and the page: `mapChip`/`pickMap`, `the map`) |
