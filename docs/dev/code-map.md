...
| a melee tool's press: the wedge the blade sweeps at this draw, every bit in the plan as one cut of it, and the sweep left on the snow | `TOOLS[id].melee` (`reach`/`half`), `SLASH_T`, `SLASH_REACH_MIN`, `slashReach`, `slashTool`, `slashes`, `updateSlashes` | `tools & bits` › `the cut` |
