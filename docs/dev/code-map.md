...
| the M map: the chart's class map, its flatten and its resample into the slot, the inks, the marks and the live legend, and the chart point -> world tile inverse a map order needs | `buildMapPanel`, `buildWorldMapImg` (throttled to `MM_REBUILD` ticks), `chartGround`, `chartSpan`, `CHART_INK`/`CHART_RIM`/`CHART_NEED`/`CHART_DARK`, `renderWorldMap`, `mapTileAt` | `world map (M)` (the class a tile files under comes from `objChart(o)`: `world`, world.js; the marks: `drawMap*`, draw-world.js) |
