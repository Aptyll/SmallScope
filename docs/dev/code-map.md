...
| the Steam transport under the wrapper: the lobby made or joined, its owner the host, chat updates as peer/gone, packets and their parts (bytes as base64 under `bin`), the lobby's data as the room's listing (roomData), and the rooms screen's feed of lobbies | `STEAM_LIST_T`, `steamB64`/`steamUnB64`, `steamRooms`, `STEAM_CHUNK`, `STEAM_LOBBY_MAX`, `steamTransport` | `steam transport` (js/net/transport-steam.js) |
