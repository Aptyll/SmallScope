...
| how a bot covers ground: the zipline ride folded into every walk - is the cable worth it, the mount point, the hold along it and the let-go at the exit, the ride no rung wanted | `ZIP_AI_GAIN`/`ZIP_AI_OFF`, `aiWalkT`, `aiZipWorth`, `aiZipMount`, `steerTo`/`walkTo` (inside `aiThink`), `ai.zipUsed` (`updateAI`) | `ai` › `the ride` |
...
| Looking for | Start at | Banner |
| --- | --- | --- |
| a pylon's sprite in each side's colour and its draw in the y-sorted pass, the cable pass (a span rasterised pixel by pixel with its sag and wind lean, over every body and canopy, under the night), and a rider's handle and rope | `PYLON_H`/`PYLON_SPRS`, `drawPylon` (dispatched from `render()`'s object pass), `ZIP_INK`/`ZIP_LIT`, `drawZips` (between `drawDropAir` and `renderLighting` in `render()`), `drawZipHandle` (from `drawPlayer`, js/draw/bodies.js) | `zipline` (the geometry it reads: `zips`/`zipPoint`/`zipLift`, the `zipline` banner, world.js) |
...
| the plates top-right under the minimap - the market's three and the roost's: raising one, ageing it, where it sits, its pixels | `NOTE_MAX`/`NOTE_LIFE`/`NOTE_IN`/`NOTE_FLASH`/`NOTE_OUT`/`NOTE_SLIDE`/`NOTE_FR`/`NOTE_W`/`NOTE_H`/`NOTE_PITCH`/`NOTE_GAP`, `NOTE_KIND` (the palette the feed line shares) + `NOTE_MARKS` (a STAMPED mark, named by a kind's `glyph` - the roost's bird), `raiseNotice`, `ageNotices` (called from `updateFx`, sim.js), `noteRect`, `renderNotices` (called from `renderUI`, js/ui/compose.js) | `notices` |
