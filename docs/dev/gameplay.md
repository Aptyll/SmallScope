...
the same ride, and `landmarks` is one of the `SAVE_ROOTS` ([saves](#saved-matches)), so a saved
match keeps the spots' clocks and a ride in progress. The body draws in its standing pose with the sled over its legs
